...
class ContrastiveTextHighlighter(object):
    ...
    def __init__(self, max_ngram_size, rgb, top_k = 10):
        """
    ...
    def highlight(self, text, keywords):
        ...
        keyword_scores: dict of keywords and scores
        """
        ...
        if(len(keywords) > 0):
            ...
            if self.max_ngram_size == 1:
                n_text = self.format_one_gram_text(text, kw_list, keywords)
            elif self.max_ngram_size > 1:
                n_text = self.format_n_gram_text(text, kw_list, keywords, self.max_ngram_size)
    ...
    def format_n_gram_text(self, text, relevant_words_array, keywords, n_gram):
        text_tokens = text.replace('\n',' ').split(' ')
        relevant_words_array = [kw.lower() for kw in relevant_words_array]
        ...
        while y < len(text_tokens):
            ...
            if n_gram_word_list:
                ...
                if len(n_gram_word_list[0].split(' ')) == 1:
                    y, new_expression = self.replace_token(text_tokens, y, n_gram_word_list, keywords)
                    ...
                    if kw_list.index(min_score_word) == 0:
                        ...
                        y, new_expression = self.replace_token(text_tokens, y, term_list, keywords)
                        final_splited_text.append(new_expression)
